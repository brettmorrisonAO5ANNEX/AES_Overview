...
class FFT_Scene(Scene):
    def construct(self):
        ...
        standard_field_def = MathTex(r"F = \left\{ a,\ b,\ c,\ d,\ e,\ f,\ ... \right\}").scale(0.75).move_to(
        ...
        axioms_rect = RoundedRectangle(corner_radius=0.25, height=6.5, width=7.5, 
                                      color=custom_colors.FOREGROUND_1, fill_opacity=0).shift(RIGHT * 2.5)
        axioms_label = MathTex(r"\forall a,b,c \in F").scale(0.75)
        ...
        self.play(standard_field.animate.shift(LEFT * 4),
                  sub_elems.animate.shift(LEFT * 4),
                  standard_field_def.animate.shift(LEFT * 4))
        ...
        add_id = MathTex(r"a+0=a").scale(scale_factor).next_to(mult_comm, DOWN, buff=0.5).align_to(mult_comm, LEFT)
        ...
        add_inv = MathTex(r"a+(-a)=0").scale(scale_factor).next_to(mult_id, DOWN, buff=0.5).align_to(mult_id, LEFT)
        ...
        distributivity = MathTex(r"a \cdot (b+c)=(a \cdot b)+(a \cdot c)").scale(scale_factor).next_to(mult_inv, DOWN, buff=0.5).align_to(mult_inv, LEFT)
